...
```bash
my-first-sam-app$ sam build --use-container
```
...
```bash
my-first-sam-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
my-first-sam-app$ sam local start-api
my-first-sam-app$ curl http://localhost:3000/
```
...
```bash
my-first-sam-app$ sam logs -n HelloWorldFunction --stack-name my-first-sam-app --tail
```
...
```bash
my-first-sam-app$ pip install -r tests/requirements.txt --user
# unit test
my-first-sam-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
my-first-sam-app$ AWS_SAM_STACK_NAME="my-first-sam-app" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "my-first-sam-app"
```
